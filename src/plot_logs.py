...
def process_single_log(csv_path):
    ...
    ax1.plot(df['frame'], df['prob_top1'], label='Probabilidade 1º Lugar', color='green', linewidth=2)
    ax1.plot(df['frame'], df['prob_top2'], label='Probabilidade 2º Lugar', color='orange', linewidth=1.5, linestyle='--')
    ax1.plot(df['frame'], df['prob_top3'], label='Probabilidade 3º Lugar', color='red', linewidth=1.5, linestyle=':')
    ...
    ax2.fill_between(df['frame'], df['margin_top1_top2'], color='skyblue', alpha=0.4)
    ax2.plot(df['frame'], df['margin_top1_top2'], color='dodgerblue', label='Margem Top1 - Top2', linewidth=2)
    ...
    if len(top1_classes) > 0:
        ...
        ax3.scatter(df_valid['frame'], y_vals, c='purple', alpha=0.7, label='Letra Detectada (Top 1)')
        ...
        if 'smoothed_prediction' in df.columns:
            ...
            ax3.plot(df_smoothed['frame'], y_smoothed, c='gold', linewidth=2, label='Predição Suavizada (Votação)')
    ...
    ax3.set_xlabel("Número do Frame")
    ...
    # Fechar a figura para liberar memória ao processar múltiplos arquivos
    plt.close(fig)
